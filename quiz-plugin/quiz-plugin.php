...
function quiz_add_results_menu()
{
    ...
    add_submenu_page(
        'quiz_results',              // Parent Slug (quiz_results is the parent)
        'Quiz Coupon Codes',         // Page Title
        'Coupon Codes',              // Menu Title
        'manage_options',            // Capability
        'quiz_coupon_codes',         // Menu Slug
        'quiz_display_coupon_codes_page' // Function to display the page content
    );
}
...
// Add 'Start Quiz' Button Before Checkout Form
function quiz_add_start_button()
{
    echo '<button id="start-quiz" class="button">Start Quiz</button>';
    echo '<div id="quiz-popup" style="display:none;">
...
add_action('woocommerce_before_checkout_form', 'quiz_add_start_button');
...
// old working code for registering suser:
...
function register_user_for_quiz()
{
    ...
    // Make sure data exists before processing
    if (!empty($user_data)) {
        // Check if the customer_id already exists in the account_numbers table
        $customer_id = sanitize_text_field($user_data['customer_id']);
        $table_name = $wpdb->prefix . 'account_numbers';
        $existing_customer = $wpdb->get_var(
            $wpdb->prepare("SELECT id FROM $table_name WHERE account_number  = %s", $customer_id)
        );

        if ($existing_customer) {
            wp_send_json_error(array('message' => 'Customer ID already assigned.'));
            return; // Exit the function if customer_id already exists
        }
        ...
        // Check for errors in inserting the user
        if ($wpdb->insert_id) {
            wp_send_json_success(array('message' => 'User successfully registered.'));
        } else {
            wp_send_json_error(array('message' => 'Failed to register user.'));
...
// Check nonce validity to prevent unauthorized requests
