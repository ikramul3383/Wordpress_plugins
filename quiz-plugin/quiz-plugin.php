...
function register_user_for_quiz()
{
    ...
    // Make sure data exists before processing
    if (!empty($user_data)) {
        // Check if the customer_id exists in the account_numbers table
        $customer_id = sanitize_text_field($user_data['customer_id']);
        $table_name = $wpdb->prefix . 'account_numbers';

        // Look for the customer_id in the account_numbers table
        ...
        if (!$customer_exists) {
            ...
            return; // Exit the function if the customer_id doesn't exist
        }
